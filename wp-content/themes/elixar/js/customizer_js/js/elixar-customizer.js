/*!
 * elixar-customizer.js
 *
 * To handle customizer sections.
 *
 * @package Elixar
 */
jQuery(document).ready(function($) {
    ...
    $('body').on('click', '.elixar-icon-list li', function(){
        var icon_class = $(this).find('i').attr('class');
        $(this).addClass('icon-active').siblings().removeClass('icon-active');
        $(this).parent('.elixar-icon-list').prev('.elixar-selected-icon').children('i').attr('class','').addClass(icon_class);
        $(this).parent('.elixar-icon-list').next('input').val(icon_class).trigger('change');
    });
    $('body').on('click', '.elixar-selected-icon', function(){
        $(this).next().slideToggle();
    });
	//Chosen JS
    $(".wh-chosen-select").chosen({
        width: "100%"
    });
});	
( function( api ) {
    // Extends our custom "example-1" section.
    api.sectionConstructor['elixar-premium'] = api.Section.extend( {
...
( function( api , $ ) {
	api.controlConstructor['repeatable'] = api.Control.extend( {
		...
		_init: function() {
            ...
            if ( ! window._upload_elix ) {
                window._upload_elix = wp.media({
            ...
            window._upload_elix.on('close', function () {
                // get selections and save to hidden input plus other AJAX stuff etc.
                var selection = window._upload_elix.state().get('selection');
                // console.log(selection);
            });
            window.media_current = {};
            window.media_btn = {};
            window._upload_elix.on('select', function () {
                // Grab our attachment selection and construct a JSON representation of the model.
                var media_attachment = window._upload_elix.state().get('selection').first().toJSON();
            ...
            control.actions = function ($context) {
                if (control.params.live_title_id) {
                    ...
                    if (control.id === 'elixar_section_order_styling') {
    ...
    function _the_editor( container ){
        var _editor = {
            ...
            ready: function( container ) {
                ...
                control.container.addClass( 'elixar-editor-added' );
...
jQuery( window ).ready( function( $ ){
    if ( typeof elixar_customizer_settings !== "undefined" ) {
        if (elixar_customizer_settings.number_action > 0) {
            $('.control-section-themes h3.accordion-section-title').append('<a class="theme-action-count" href="' + elixar_customizer_settings.action_url + '">' + elixar_customizer_settings.number_action + '</a>');
        }
    }
    /**
     * For Hero layout content settings
     */
    $( 'select[data-customize-setting-link="elixar_hero_layout"]').on( 'change on_custom_load', function(){
        var v = $( this).val() || '';
        $( "li[id^='customize-control-elixar_hcl']").hide();
        $( "li[id^='customize-control-elixar_hcl"+v+"']").show();
    } );
    $( 'select[data-customize-setting-link="elixar_hero_layout"]').trigger( 'on_custom_load' );
    /**
     * For Gallery content settings
     */
    $( 'select[data-customize-setting-link="elixar_gallery_source"]').on( 'change on_custom_load', function(){
        var v = $( this).val() || '';
        $( "li[id^='customize-control-elixar_gallery_source_']").hide();
        $( "li[id^='customize-control-elixar_gallery_api_']").hide();
        $( "li[id^='customize-control-elixar_gallery_settings_']").hide();
        $( "li[id^='customize-control-elixar_gallery_source_"+v+"']").show();
        $( "li[id^='customize-control-elixar_gallery_api_"+v+"']").show();
        $( "li[id^='customize-control-elixar_gallery_settings_"+v+"']").show();
    } );
    $( 'select[data-customize-setting-link="elixar_gallery_source"]').trigger( 'on_custom_load' );
    /**
     * For Gallery display settings
     */
    $( 'select[data-customize-setting-link="elixar_gallery_display"]').on( 'change on_custom_load', function(){
        var v = $( this).val() || '';
        switch ( v ) {
            case 'slider':
                $( "#customize-control-elixar_g_row_height, #customize-control-elixar_g_col, #customize-control-elixar_g_spacing, #customize-control-elixar_g_grid_col").hide();
                break;
            case 'carousel':
                $( "#customize-control-elixar_g_row_height, #customize-control-elixar_g_grid_col").hide();
                $( "#customize-control-elixar_g_col, #customize-control-elixar_g_spacing").show();
                break;
            case 'masonry':
                $( "#customize-control-elixar_g_row_height, #customize-control-elixar_g_grid_col").hide();
                $( "#customize-control-elixar_g_col, #customize-control-elixar_g_spacing").show();
                break;
            default:
                $( "#customize-control-elixar_g_col, #customize-control-elixar_g_row_height").hide();
                $( "#customize-control-elixar_g_spacing, #customize-control-elixar_g_grid_col").show();
        }
    } );
    $( 'select[data-customize-setting-link="elixar_gallery_display"]').trigger( 'on_custom_load' );
} );
/**
 * Icon picker
 */
jQuery( document ).ready( function( $ ) {
    ...
    $.each( Elixar_Icon_Picker.fonts, function( key, font ) {
        font = $.extend( {}, {
            url: '',
            name: '',
            prefix: '',
    ...
    icon_picker.find( '.c-icon-search input' ).attr( 'placeholder', Elixar_Icon_Picker.search );
    ...
    var display_footer_layout = function( l ){
        $( 'li[id^="customize-control-elixar_footer_custom_"]' ).hide();
        $( 'li[id^="customize-control-elixar_footer_custom_'+l+'_columns"]' ).show();
    };
    display_footer_layout( $( '#customize-control-elixar_footer_layout select' ).val() );
    $( '#customize-control-elixar_footer_layout select' ).on( 'change', function ()  {
        display_footer_layout( $( this ).val() );
    } );
} );
// Hero BG Video URL show hide
wp.customize( 'elixar_bg_video_url', function( setting ) {
	...
	wp.customize.control( 'elixar_video_play', setupControl );
	wp.customize.control( 'elixar_video_volume', setupControl );
	wp.customize.control( 'elixar_video_controls', setupControl );
} );