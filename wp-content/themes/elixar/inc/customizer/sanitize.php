 ...
 * @package elixar
 */
function elixar_sanitize_css($string) {
    $string = preg_replace( '@<(script|style)[^>]*?>.*?</\\1>@si', '', $string );
    $string = strip_tags($string);
    return trim( $string );
}
function elixar_sanitize_color_alpha( $color ){
 ...
 */
function elixar_sanitize_repeatable_data_field( $input , $setting ){
            ...
            if ( isset( $fields[ $id ] ) ){
                switch( strtolower( $fields[ $id ]['type'] ) ) {
                    ...
                    case 'color':
                        ...
                        break;
                    case 'coloralpha':
                        $data[ $i ][ $id ] = elixar_sanitize_color_alpha( $value );
                        break;
                    case 'checkbox':
                        $data[ $i ][ $id ] =  elixar_sanitize_checkbox( $value );
                        break;
                    case 'select':
                        $data[ $i ][ $id ] = '';
...
function elixar_sanitize_file_url( $file_url ) {
    $output = '';
 ...
 * @return bool
 */
function elixar_hero_fullscreen_callback ( $control ) {
    if ( $control->manager->get_setting('elixar_hero_fullscreen')->value() == '' ) {
        return true;
    } else {
        return false;
    }
}
function elixar_sanitize_select( $input, $setting ){
...
/**
 *  Elixar Sanitize Functions.
 */
if ( ! function_exists( 'elixar_sanitize_checkbox' ) ) {
    function elixar_sanitize_checkbox( $input ) {
        if ( $input == 1 ) {
            return 1;
        } else {
            return 0;
...
function elixar_sanitize_hex_color( $color ) {
    if ( $color === '' ) {
        return '';
    }
    if ( preg_match('|^#([A-Fa-f0-9]{3}){1,2}$|', $color ) ) {
...
function elixar_showon_frontpage() {
    return is_page_template( 'home-page.php' );
}
function elixar_sanitize_array_string($array){
    $new_array = array();
    if(is_array($array) && !empty($array)){
        foreach ($array as $key => $value) {
            array_push($new_array, esc_attr($value));
        }
        return $new_array;
    }else{
...
//Icon Sanitize
function elixar_sanitize_text( $input ) {
